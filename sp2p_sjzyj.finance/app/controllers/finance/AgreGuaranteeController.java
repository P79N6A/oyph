...
public class AgreGuaranteeController extends BaseController {
	 ...
	 */
	public static void createProductInformation() {
		// 清除数据库数据
		...
		for (t_pact pact : pactAll) {
			...
			if (bid.invest_expire_time != null) {
				...
				// 募集起始日期
				t_agre_guarantee.setCollect_start_date(bid.time);
				// 募集结束日期
				t_agre_guarantee.setCollect_end_date(bid.invest_expire_time);
				// 产品预计终止日期 募集起始日期10天后
				t_agre_guarantee.setIntending_end_date(DateUtil.addDay(bid.time, 10));
				// 产品说明
				t_agre_guarantee.setPrd_remark(bid.title);
				// 担保人证据类型
				t_agre_guarantee.setGuar_id_typ("1");
	...
	public static void createXml() {
		try {
			...
			// 6、生成xml文件
			File file = new File("data/day/FP.FP07.001.xml");
