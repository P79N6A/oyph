...
public class MallScroeRule {
	 ...
	 * @return
	 */
	public static PageBean<t_mall_scroe_rule> queryMallScroeRuleByPage(int currPage, int pageSize, ResultInfo error) {
		...
		StringBuffer sql_list = new StringBuffer("select new t_mall_scroe_rule(id, time, type, scroe, status,amount) from t_mall_scroe_rule t where 1=1");
	 ...
	 * @return
	 */
	public static int updateRules(int signScroe,int registerScore,int amount,int investScore){
		
		int rows =0;
		String sql1 ="update t_mall_scroe_rule set scroe=? where type=1";
		String sql2 ="update t_mall_scroe_rule set scroe=? where type=2";
		String sql3 ="update t_mall_scroe_rule set scroe=?,amount=? where type=3";
		
	
		rows = JPA.em().createQuery(sql1).setParameter(1, signScroe).executeUpdate();
		rows = JPA.em().createQuery(sql2).setParameter(1, registerScore).executeUpdate();
		rows = JPA.em().createQuery(sql3).setParameter(1, investScore).setParameter(2, amount).executeUpdate();
		return rows;
	}
